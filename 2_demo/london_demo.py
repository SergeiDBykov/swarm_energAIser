...
house_number = st.slider(label='Select the house_number for aggregating energy use:', min_value=50, max_value=400, value=200, step=50)
...
lags_horizon = 24
model_naive = val.shift(+24*7) #repeat value from 24*7 hours ago
model_naive_train = train.shift(+24*7) 
model_catb = CatBoostModel(lags_future_covariates = (lags_horizon, lags_horizon) , lags_past_covariates=lags_horizon,
                    )#learning_rate = 0.007, num_trees = 1000, early_stopping_rounds = 5)
model_lgbm = LightGBMModel(lags_future_covariates = (lags_horizon, lags_horizon) , lags_past_covariates=lags_horizon,
                    )#learning_rate = 0.007, num_trees = 1000, early_stopping_rounds = 5)
cov_args = {"future_covariates": [datetime_covatiates,hodidays_covariates],
        "past_covariates": [frequency_covariates]}
model_catb.fit(train, **cov_args)
model_lgbm.fit(train, **cov_args)
...
# Plot of modeling results (static plot in plotly)
